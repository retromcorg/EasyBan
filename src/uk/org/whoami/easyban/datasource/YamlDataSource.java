...
public class YamlDataSource extends Configuration implements DataSource {
    ...
    public synchronized boolean isIpBanned(final String ip) {
        for (final String bannedNick : this.bans.keySet()) {
            for (Map.Entry<String, List<String>> ban : this.history.entrySet()) {
                if (ban.getKey() != null && ban.getKey().equalsIgnoreCase(bannedNick)) {
                    if (ban.getValue() != null && ban.getValue().contains(ip)) {
                        return true;
                    }
                }
            }
